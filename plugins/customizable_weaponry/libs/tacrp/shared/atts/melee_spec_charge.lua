...
ATT.Pros = { "RELOAD: Charge forwards", "WALK + RELOAD: Select charge mode", "Damage resistance during charge",
...
local function ChargeFraction(ply)
	...
	return 1 -
		math.Clamp(
			(ply:GetNWFloat("TacRPChargeTime", 0) + chargestats(ply, stat_dur) - CurTime() - g) /
			(chargestats(ply, stat_dur) - g),
			0, 1)
end
...
ATT.Hook_PreShoot = function(wep)
	...
	if incharge(ply) then
		exitcharge(ply, true)
		ply:EmitSound("PLUGIN.Charge.End")
		chargemelee(wep)
		return true
...
ATT.Hook_PreReload = function(wep)
	...
	if SERVER then
		ply:EmitSound("PLUGIN.Charge.Windup", 80)
	end
...
function ATT.TacticalDraw(self)
	...
	if ! chargin and ply:KeyDown(IN_WALK) then
		local w_mode = w / modecount - PLUGIN.SS(0.5) * (modecount - 1)
		for i = 0, modecount - 1 do
			local c_bg, c_cnr, c_txt = PLUGIN.GetPanelColors(curmode == i, curmode == i)
			surface.SetDrawColor(c_bg)
			PLUGIN.DrawCorneredBox(x + (i / modecount * w) + (i * PLUGIN.SS(0.5)), y - PLUGIN.SS(45), w_mode,
				PLUGIN.SS(6),
				c_cnr)
			draw.SimpleText(modes[i][1], "TacRP_HD44780A00_5x8_4",
				x + (i / modecount * w) + (i * PLUGIN.SS(0.5)) + w_mode / 2,
				y - PLUGIN.SS(45 - 3), c_txt, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		end

		surface.SetDrawColor(0, 0, 0, 240)
		PLUGIN.DrawCorneredBox(x, y - PLUGIN.SS(38), w, PLUGIN.SS(24))

		local desc = PLUGIN.MultiLineText(modes[curmode][2], w - PLUGIN.SS(6), "TacRP_Myriad_Pro_6")
		for j, text in ipairs(desc or {}) do
			draw.SimpleText(text, "TacRP_Myriad_Pro_6", x + PLUGIN.SS(3),
				y - PLUGIN.SS(38 - 2) + (j - 1) * PLUGIN.SS(6.5),
	...
	local c_bg, c_cnr, c_txt = PLUGIN.GetPanelColors(chargin, chargin)
	surface.SetDrawColor(c_bg)
	PLUGIN.DrawCorneredBox(x + w / 2 - w / 6, y - PLUGIN.SS(12), w / 3, PLUGIN.SS(9), c_cnr)
	draw.SimpleText(modes[curmode][1], "TacRP_HD44780A00_5x8_6", x + w / 2, y - PLUGIN.SS(8), c_txt, TEXT_ALIGN_CENTER,
		TEXT_ALIGN_CENTER)

	if chargin then
		c = math.Clamp(
			(ply:GetNWFloat("TacRPChargeTime", 0) + chargestats(ply, stat_dur) - CurTime()) / chargestats(ply, stat_dur),
			0, 1)
...
hook.Add("StartCommand", "TacRP_Charge", function(ply, cmd)
	if incharge(ply) then
		...
		cmd:SetButtons(bit.band(cmd:GetButtons(),
			bit.bnot(IN_DUCK + IN_JUMP + IN_FORWARD + IN_BACK + IN_MOVELEFT + IN_MOVERIGHT + IN_WALK + IN_SPEED)))
			...
			if ! active or vel < chargestats(ply, stat_vel) * 0.4 or IsValid(ply.TacRPTryCollide) then
				...
				if IsValid(ent) and ent:GetOwner() ~= ply then
					if SERVER then
						...
						if ent:IsPlayer() or ent:IsNPC() or ent:IsNextBot() then
							...
							grace = (ent.TacRPNextChargeHit or 0) <= CurTime() and
								ent:Health() <= dmginfo:GetDamage() * dmgscale

							if grace then
								mv:SetVelocity(ply:GetForward() * chargestats(ply, stat_vel))
							elseif d >= 0.5 then
								ply:EmitSound("PLUGIN.Charge.HitFlesh_Range", 80)
							else
								ply:EmitSound("PLUGIN.Charge.HitFlesh", 80)
							end
						elseif IsValid(phys) then
							...
							phys:ApplyForceOffset(
								ply:GetForward() * chargestats(ply, stat_vel) * (d * 0.5 + 0.5) * dmgscale * 10,
								ply:GetPos())

							grace = phys:IsMotionEnabled() and
								((ent.TacRPNextChargeHit or 0) <= CurTime() or (ply.TacRPGrace or 0) >= CurTime())
							if ! grace then
								ply:EmitSound("PLUGIN.Charge.HitWorld", 80)
				...
				if ! active or (! grace and ((tr and tr.Hit) or (ply.TacRPChargeTrace and IsValid(ent) and (ent:IsNPC() or ent:IsPlayer() or ent:IsNextBot())))) then
					...
					if tr.Hit then
						...
						ply:EmitSound("PLUGIN.Charge.End") -- interrupt sound
					end
