local PLUGIN = PLUGIN
function PLUGIN.DoorBust(ent, vel, attacker)
  if not string.find(ent:GetClass(), "door") then return end
  local cvar = 1 --PLUGIN.ConVars["doorbust"]:GetInt()
  local t = 300 -- PLUGIN.ConVars["doorbust_time"]:GetFloat()
  if cvar == 0 or ent.exp_tacrp_DoorBusted then return end
  ent.exp_tacrp_DoorBusted = true
  ...
  if ent:GetPhysicsObject():IsValid() and cvar == 1 then
    ...
    timer.Create("TacRP_DoorBust_" .. ent:EntIndex(), t, 1, function()
      if IsValid(ent) then
        ...
        ent.exp_tacrp_DoorBusted = false
