...
function SWEP:GetReloadTime(base)
  ...
  if not valfunc(self, "ShotgunReload") then
    local seq = vm:LookupSequence(self:TranslateSequence("reload"))
    local basetime = vm:SequenceDuration(seq)
    local mult = valfunc(self, "ReloadTimeMult") / PLUGIN.ConVars["mult_reloadspeed"]:GetFloat()
...
function SWEP:GetBestFiremode(base)
  ...
  if valfunc(self, "Firemodes") then
    ...
    for k, v in pairs(valfunc(self, "Firemodes")) do
      if not bfm or v == 2 or (bfm <= 1 and v < bfm) then
        bfm = v
        bfm_i = k
      end
    end
...
function SWEP:GetSubClassName(tier)
  if self.SubCatType then
    local type_txt = PLUGIN:TryTranslate(self.SubCatType)
    if tier and self.SubCatTier and self.SubCatTier ~= "9Special" then
      type_txt = PLUGIN:GetPhrase("cust.type_tier", { tier = PLUGIN:TryTranslate(self.SubCatTier), type = type_txt })
    end
    return type_txt
  end
  return "Weapon"
...
SWEP.StatGroups = {
  {
    ...
    RatingFunction = function(self, base)
      ...
      local ttt = PLUGIN.GetBalanceMode() == PLUGIN.BALANCE_TTT
      local pve = PLUGIN.GetBalanceMode() == PLUGIN.BALANCE_PVE
  ...
  },
  {
    ...
    RatingFunction = function(self, base)
      ...
      local dot = dmg_avg * num /
          (60 / erpm + self:GetReloadTime(base) / (valfunc(self, "ClipSize") / valfunc(self, "AmmoPerShot")))
      local dps_s, dot_s
      if pve then
  ...
  },
  {
    ...
    RatingFunction = function(self, base)
      ...
      local pbd = valfunc(self, "AutoBurst") and valfunc(self, "PostBurstDelay") or
          math.max(0.15, valfunc(self, "PostBurstDelay"))
      if bfm == 1 then
  ...
  },
  {
    ...
    RatingFunction = function(self, base)
      ...
      local bonus = 50
      local freeaim_s = 1
      if PLUGIN.ConVars["freeaim"]:GetBool() then
      ...
      local sway_s = 1
      if PLUGIN.ConVars["sway"]:GetBool() then
...
SWEP.StatDisplay = {
  ...
  },
  {
    ...
    AggregateFunction = function(self, base, val)
      if not (self:IsDamageConstant(false) and self:IsDamageConstant(true)) then return end
      -- local valfunc = base and self.GetBaseValue or self.GetValue
  ...
  },
  {
    ...
    Value = "RPMMultBurst",
    AggregateFunction = function(self, base, val)
      if not self:HasFiremode(-1) then return end
      local valfunc = base and self.GetBaseValue or self.GetValue
  ...
  -- For use when in "Bloom Modifies Recoil"
  {
    ...
    AggregateFunction = function(self, base, val)
      return math.Round(
        val * (base and self:GetBaseValue("RecoilAltMultiplier") or self:GetValue("RecoilAltMultiplier")), 2)
    end,
    Unit = nil,
    Value = "RecoilSpreadPenalty",
    LowerIsBetter = true,
