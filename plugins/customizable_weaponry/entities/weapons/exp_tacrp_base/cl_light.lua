...
function SWEP:CreateFlashlights()
  ...
  for i, k in pairs(self.Attachments) do
    ...
    if atttbl.Flashlight then
      ...
      if not IsValid(l) then continue end
      ...
      table.insert(PLUGIN.FlashlightPile, g_light)
    end
  end
...
function SWEP:KillFlashlightsVM()
  if not self.Flashlights then return end
...
function SWEP:DrawFlashlightsVM()
  if not self:GetTactical() then
    self:KillFlashlights()
    return
  end

  if not self.Flashlights then
...
function SWEP:DrawFlashlightGlare(pos, ang, strength, dot)
  ...
  if PLUGIN.ConVars["flashlight_blind"]:GetBool() then
    dot = dot ^ 4
    ...
    if IsValid(wep) and wep.ArcticTacRP and wep:IsInScope() and wep:GetValue("ScopeOverlay") then
      s = s +
          math.Clamp(1 - diff:Length() / 4096, 0, 1) ^ 1.2 * wep:GetSightAmount() * dot * 3000 *
          math.Rand(0.95, 1.05)
    end
...
function SWEP:DoScopeGlint()
  --if self:GetOwner() == LocalPlayer() then return end
  if not PLUGIN.ConVars["glint"]:GetBool() then return end
  if not self:GetValue("ScopeOverlay") then return end
  ...
  local strength = dot * math.Clamp((diff:Length() - 1024) / 3072, 0, 3) *
      math.Clamp(90 / self:GetValue("ScopeFOV") / 10, 0, 1)
...
function SWEP:DoMuzzleLight()
  if (not IsFirstTimePredicted() and not game.SinglePlayer()) or not PLUGIN.ConVars["muzzlelight"]:GetBool() then return end
