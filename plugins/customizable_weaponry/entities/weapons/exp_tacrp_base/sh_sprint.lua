...
function SWEP:GetIsSprinting()
  ...
  if not owner:IsValid() or owner:IsNPC() or owner:IsNextBot() then
    return false
  end

  if PLUGIN.ConVars["sprint_counts_midair"]:GetBool() and owner:GetMoveType() ~= MOVETYPE_NOCLIP and not owner:IsOnGround() and not self:GetReloading() then
    return true
  end

  if self:DoForceSightsBehavior() and self:GetScopeLevel() == 0 and not self:GetInBipod() and self:GetBlindFireMode() == PLUGIN.BLINDFIRE_NONE then
    return true
  end
  ...
  if owner.exp_tacrp_SprintBlock then return false end
  ...
  if TTT2 then
    if SPRINT and SPRINT:IsSprinting(owner) then
      ...
      return owner.isSprinting == true and (owner.sprintProgress or 0) > 0 and owner:KeyDown(IN_SPEED) and
          not owner:Crouching() and owner:OnGround()
    end
  end
  ...
  if not owner.exp_tacrp_Moving then return false end  -- Don't check IN_ move keys because 1) controllers and 2) bots
  if not owner:KeyDown(IN_SPEED) then return false end -- SetButtons does not seem to affect this?
...
function SWEP:CanStopSprinting()
  ...
  if PLUGIN.ConVars["sprint_counts_midair"]:GetBool() and owner:GetMoveType() ~= MOVETYPE_NOCLIP and not owner:OnGround() and not self:GetReloading() then
    return false
  end
...
function SWEP:EnterSprint()
  if not self:CanShootInSprint() then
    self:ToggleBlindFire(PLUGIN.BLINDFIRE_NONE)
  end
  if not self:CanReloadInSprint() and self:GetReloading() then
...
function SWEP:ThinkSprint()
  ...
  if self.LastWasSprinting and not sprinting then
    self:ExitSprint()
  elseif not self.LastWasSprinting and sprinting then
    self:EnterSprint()
  end

  self.LastWasSprinting = sprinting

  if sprinting and not self:GetInBipod() then
...
function SWEP:CanShootInSprint(base)
  if not PLUGIN.ConVars["sprint_lower"]:GetBool() then return true end
  if base then
...
function SWEP:CanReloadInSprint(base)
  return PLUGIN.ConVars["sprint_reload"]:GetBool()
end

function SWEP:DoForceSightsBehavior()
  return PLUGIN.ConVars["sightsonly"]:GetBool() and self:GetValue("Scope")
end
