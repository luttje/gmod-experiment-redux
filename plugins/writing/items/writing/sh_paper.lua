...
ITEM.functions.View = {
	OnRun = function(item)
		netstream.Start(item.client, "ixViewPaper", item:GetID(), item:GetData("text", ""), 0)
		return false
	end,
...
ITEM.functions.Edit = {
	OnRun = function(item)
		netstream.Start(item.client, "ixViewPaper", item:GetID(), item:GetData("text", ""), 1)
		return false
	end,
	...
	OnCanRun = function(item)
		...
		return owner == 0 or owner == item.client:GetCharacter():GetID() and item:GetData("text", "") == ""
	end
}
...
ITEM.functions.take.OnCanRun = function(item)
	...
	return IsValid(item.entity) and (owner == 0 or owner == item.client:GetCharacter():GetID())
end
